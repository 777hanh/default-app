...
import solidUrl from '~/assets/icons/js/solid.min.js?url';
...
import allCssUrl from '~/assets/icons/css/all.min.css?url';

/**
 * Composable to load all FontAwesome icon libraries
 * Automatically imports all icon sets defined in the assets/icons/js directory
 * Includes defer attribute to prevent blocking page render
 */

export const useFontAwesome = () => {
	useHead({
		link: [{ rel: 'stylesheet', href: allCssUrl }],
		script: [
