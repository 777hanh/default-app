...
axiosInstance.interceptors.request.use((config) => {
	...
	const locale =
		...
		JSON.parse(localStorage.getItem('listLanguage')).filter((x) => x.id === localStorage.getItem('currentLanguage')).length > 0
			? localStorage.getItem('currentLanguage')
			: defaultLocale;
	if (locale) {
...
const Repository = {
	...
	postcancel: (url, data, onUploadProgress) => {
	...
	uploadFileProgress: (url, data, onUploadProgress) => {
